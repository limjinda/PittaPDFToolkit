...
export function EncryptTool({ onViewChange }: EncryptToolProps) {
  ...
  async function handleEncrypt() {
    if (!inputPath || !userPassword) return;

    try {
      const outPath = await save({
        title: "Save Protected PDF",
        defaultPath: inputPath.replace(/\.pdf$/i, "") + "_protected.pdf",
        filters: [{ name: "PDF Files", extensions: ["pdf"] }],
      });
      ...
      // If owner password is empty, set it to the same as user password
      ...
      try {
        ...
        useWorkspaceStore.getState().createWorkspace([{
          path: outPath,
          name: outPath.split(/[\\/]/).pop() || "protected.pdf",
          bytes,
          numPages: doc.numPages
        }]);
        if (onViewChange) onViewChange("viewer");
      } catch (err) {
        console.error("Failed to auto-open protected PDF:", err);
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="text-sm text-muted-foreground">
        Encrypt your PDF with a password to restrict opening, copying, or modifying permissions.
      </div>
      ...
      <Button
        ...
        disabled={!inputPath || !userPassword.trim() || status === "running"}
        className="w-full"
      >
        {status === "running" ? "Encrypting..." : "Protect PDF"}
      </Button>
      ...
      {status === "running" && (
        <div className="flex flex-col gap-2 animate-pulse">
          ...
          <div className="text-xs text-center text-muted-foreground">
            Encrypting your PDF... Please do not close the app.
          </div>
          <style>{`
            @keyframes loading {
              0% { left: -30%; width: 30%; }
      ...
      {status === "success" && (
        <div className="p-4 bg-green-500/10 border border-green-500/20 text-green-500 rounded-lg text-sm flex flex-col gap-2">
          <div className="font-semibold flex items-center gap-1.5">
            <span>✓</span> PDF encrypted successfully!
          </div>
